...
# locations <- data.frame(x = -173.9335093, 
#                         y = 52.1756401, 
#                         label = "Nazan Bay", 
#                         region = "ai", 
#                         focal_buffer = 15000, 
...
region_towpath <- sf::st_read(here::here("analysis", "efh_bathymetry_2028", "data", "bts_tows", paste0(locations$region, "_towpath.shp"))) |>
...
plot_bbox <- sf::st_buffer(focal_zone, dist = 15000) |>
  sf::st_bbox()

focal_bbox <- sf::st_buffer(focal_zone, dist = 15000) |>
  bbox_to_polygon()

expanded_bbox <- sf::st_buffer(focal_zone, dist = 100000) |>
  bbox_to_polygon()
...
# Calculate BPI following Lundblad et al. (2006) ----
...
plot_focal_bathy <- ggplot() +
  ...
  scale_fill_distiller(name = "Depth (m)", na.value = "grey70", direction = 1) +
...
plot_expanded_bpi_2023_method <- ggplot() +
  geom_stars(data = stars::st_as_stars(
    terra::crop(bpi_expanded_2023_method, focal_bbox)
    )
    ) +
  geom_sf(data = map_layers$akland, fill = "grey30", color = NA) +
  geom_sf(data = region_towpath,
...
plot_expanded_bpi_2023_diff <- ggplot() +
  ...
  geom_sf(data = region_towpath,
          mapping = aes(color = PERFORMANCE_LABEL)) +
  scale_color_manual(name = "Performance", values = performance_color) +
  scale_fill_distiller(name = "Extended-Focal", na.value = "grey70", direction = 1, palette = "PuOr") +
  coord_sf(xlim = c(plot_bbox['xmin'], plot_bbox['xmax']) + c(-3000, 3000),
           ylim = c(plot_bbox['ymin'], plot_bbox['ymax']) + c(-3000, 3000),
           expand = 0) +
  theme_bw() +
